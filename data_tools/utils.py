...
def associate_screenshots_with_transcription(screenshots, transcription):
    ...
    for segment in trans_segments:
        ...
        if (
            screenshot_index < len(screenshots) - 1
            and segment["start"] > screenshots[screenshot_index + 1][0]
        ):
            screenshot_index += 1
        ...
        if associated_data and associated_data[-1][0] == current_screenshot_path:
            ...
            associated_data[-1] = (
                ...
                associated_data[-1][1] + " " + segment["text"],
            )
        else:
...
def process_frame(
    frame, frame_count, fps, prev_hash, hash_size, threshold, output_folder
):
...
def extract_screenshots_generator(
    video_path, output_folder="./tmp/screenshots", hash_size=8, threshold=10
):
    ...
    while success:
        frame_count += 1
        prev_hash, timestamp, screenshot_path = process_frame(
            frame, frame_count, fps, prev_hash, hash_size, threshold, output_folder
        )
        if screenshot_path:
...
def convert_videos_to_mp3(source_path, destination_path):
    ...
    videos = [
        f for f in os.listdir(source_path) if f.endswith(".mp4") or f.endswith(".webm")
    ]
...
def diarize_speakers(audio_path: str):
    ...
    transcription = model.transcribe(
        audio_path, batch_size=BATCH_SIZE, print_progress=True
    )
    ...
    return (
        whisperx.assign_word_speakers(diarization_result, aligned_transcription),
...
def text_to_speech(id, text, model_name):
    manager = ModelManager()
    model_path, config_path, model_item = manager.download_model(
        f"tts_models/de/thorsten/{model_name}"
    )
    ...
    filename = f"./data/{id}_{model_name}.wav"
